...
def most_common_vehicle_type(
    vehicles_df: pl.DataFrame, target_column: str = "KIND"
) -> tuple[str, int]:
    """
    ...
    most_common = (
        vehicles_df.group_by(target_column).len().sort("len", descending=True)[0].row()
    )
    return most_common
...
def vehicle_ownership_by_region(
    ...
    ownership_by_region = (
        vehicle_acquiry_df.with_columns(transform_koatuu_to_human)
